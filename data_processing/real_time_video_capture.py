...
ap.add_argument("-ms", "--movie_seconds", type=int, default=15,
	help="number of seconds camera to record")
...
def establish_writer(W, H, i):
	datetime_str = 'output/{}.avi'.format(i)
	print('writing to: ' + datetime_str)
	fourcc = cv2.VideoWriter_fourcc(*"MJPG")
	writer = cv2.VideoWriter(datetime_str, fourcc, 30,
		(W, H), True)
	return writer
...
def trigger_camera_subprocess():
	movie_command = '--wait-event={}s'.format(movie_seconds)
	return subprocess.Popen(['gphoto2', '--set-config', 'movie=1', movie_command])
...
def outputs_list_writer(last_datetime, x_axis_bounding_list):
	...
	if (current_datetime - last_datetime).seconds < (movie_seconds + 5):
...
# TODO:
# Set up subprocess to trigger camera - DONE
# look into camera focusing and taking a video - auto foxus lens, good on video
		...
		for i in np.arange(0, detections.shape[2]):
			...
			if CLASSES[idx] == 'person':
				if confidence > args["confidence"]:
					...
					# replace below block with directional analysis 
